---
title: Social status effects on zebrafish gut microbiomes
author: "Emily Scott, Fadi Issa, Michael Brewer, Ariane L. Peralta"
...
emmeans(chao1.lm, pairwise ~ Day)
 ...
 #Day_0 - Day_14    104.69 49.4 77   2.120  0.1559
 #Day_0 - Day_7     -11.34 48.2 77  -0.235  0.9954
 #Day_0 - Day_IP    -14.32 48.2 77  -0.297  0.9908
 #Day_14 - Day_7   -116.03 48.8 77  -2.378  0.0900
 #Day_14 - Day_IP  -119.01 48.8 77  -2.439  0.0783
 #Day_7 - Day_IP     -2.98 47.6 77  -0.063  0.9999
...
```
#Plot Richness
```{r Plot Chao1, echo=TRUE}
diversity$Day <- factor(diversity$Day,      # Reordering group factor levels
                         levels = c("Day_IP", "Day_0", "Day_7", "Day_14"))
...
p <- ggplot(diversity, aes(x=Social.Status, y=S.chao1, color=as.factor(Social.Status)))+ 
  ...
  scale_color_manual(name="Social Status", 
                     ...
                     labels = c("dominant", "subordinate", "communal", "isolate")) + 
  facet_grid(.~Day)
...
chao1 <- p + 
  ...
  labs(x = "Social Status", y = "Chao 1 Richness") + 
    theme(strip.text.x = element_text(size=14, face="italic"), strip.text.y =   
          ...
          fill="white", size=1)) +
          scale_x_discrete(breaks=c("dominant", "subordinate", "communal", "isolate"),
                           labels=c("dominant", "subordinate", "communal", "isolate")) +
...
chao1
ggsave("../figures/chao1_updated.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=8, height=5, dpi=300, limitsize=TRUE)
```
...
p <- ggplot(diversity, aes(x=Social.Status, y=shannon, color=as.factor(Social.Status)))+ 
  geom_boxplot() +
  geom_point(aes(color=factor(Social.Status)), size=2, position = position_jitterdodge()) +       
  scale_color_manual(name="Social Status", values=c("#B2182B", "#0077BB", "#9970AB", "#BBBBBB"), 
                     labels = c("dominant", "subordinate", "communal", "isolate")) + 
  facet_grid(.~Day)
  ...
  labs(x = "Social Status", y = "Shannon Diversity Index (H')") + 
    theme(strip.text.x = element_text(size=14, face="italic"), strip.text.y =   
          ...
          scale_x_discrete(breaks=c("dominant", "subordinate", "communal", "isolate"),
                           labels=c("dominant", "subordinate", "communal", "isolate"))
shannon
ggsave("../figures/shannon_updated.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=8, height=5, dpi=300, limitsize=TRUE)
```
...
p <- ggplot(diversity, aes(x=Social.Status, y=simp.even, color=as.factor(Social.Status)))+ 
  ...
  geom_point(aes(color=factor(Social.Status)), size=2, position = position_jitterdodge()) +       
  scale_color_manual(name="Social Status", values=c("#B2182B", "#0077BB", "#9970AB", "#BBBBBB"), 
                     labels = c("dominant", "subordinate", "communal", "isolate")) + facet_grid(.~Day)
  ...
  labs(x = "Social Status", y = "Simpson's Evenness") + 
    theme(strip.text.x = element_text(size=14, face="italic"), strip.text.y =   
          ...
          scale_x_discrete(breaks=c("dominant", "subordinate", "communal", "isolate"),
                           labels=c("dominant", "subordinate", "communal", "isolate"))
simpeven
ggsave("../figures/simpeven_updated.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=8, height=5, dpi=300, limitsize=TRUE)
```
...
#levels(design.type)
# [1] "1_Dominant.Day_0"     "2_Subordinate.Day_0"  "3_Communal.Day_0"     "4_Isolate.Day_0"     
# [5] "1_Dominant.Day_14"    "2_Subordinate.Day_14" "3_Communal.Day_14"    "4_Isolate.Day_14"    
# [9] "1_Dominant.Day_7"     "2_Subordinate.Day_7"  "3_Communal.Day_7"     "4_Isolate.Day_7"     
#[13] "1_Dominant.Day_IP"    "2_Subordinate.Day_IP" "3_Communal.Day_IP"    "4_Isolate.Day_IP"  
